...
class LegalAdminTermsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    for ($counter = 1; $counter <= $extras_count; $counter++) {
      $extra = $conditions['extras']['extras-' . $counter] ?? '';
  ...
  protected function legalConditionsUpdated(array $new) {
    ...
    for ($counter = 1; $counter <= $count; $counter++) {
      $previous_same_language_extra = $previous_same_language['extras']['extras-' . $counter] ?? '';
      $previous_extra               = $previous['extras']['extras-' . $counter] ?? '';
