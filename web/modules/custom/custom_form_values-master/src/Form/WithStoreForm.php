...
namespace Drupal\custom_form_values\Form;
...
class WithStoreForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $tempstore = $this->tempStoreFactory->get('custom_form_values');

    // 3. Store the $params array with the key 'params'.
    try {
      $tempstore->set('params', $params);
      // 4. Redirect to the simple controller.
      $form_state->setRedirect('custom_form_values.simple_controller_show_item');
    }
    catch (\Exception $error) {
      // Store this error in the log.
      $this->loggerFactory->get('custom_form_values')->alert(t('@err', ['@err' => $error]));
